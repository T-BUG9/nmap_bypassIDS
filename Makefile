...
CPPFLAGS = -I$(top_srcdir)/liblinear -I$(top_srcdir)/liblua -I$(top_srcdir)/libdnet-stripped/include -I$(top_srcdir)/libssh2/include -I$(top_srcdir)/libz -I$(top_srcdir)/libpcre  -I$(top_srcdir)/libpcap -I$(top_srcdir)/nbase -I$(top_srcdir)/nsock/include $(DEFS)
...
$(TARGET): build-netutil build-liblinear build-pcap build-libssh2 build-zlib build-pcre build-nsock build-nbase build-dnet build-lua \
	$(OBJS) main.o
	@echo Compiling nmap
	rm -f $@
...
clean: clean-lua clean-liblinear clean-pcap clean-pcre clean-dnet clean-libssh2 clean-zlib\
